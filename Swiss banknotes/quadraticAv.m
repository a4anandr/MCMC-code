function [thetaZv, a] = quadraticAv(theta, z)
...
for m = 1:1:n    
    grad_psi(:,:,m) = [eye(d); diag(theta(m,:)); theta(m,2) theta(m,1) 0 0; theta(m,3) 0 theta(m,1) 0; theta(m,4) 0 0 theta(m,1);0 theta(m,3) theta(m,2) 0; 0 theta(m,4) 0 theta(m,2); 0 0 theta(m,4) theta(m,3)];
